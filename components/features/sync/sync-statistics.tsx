...
export function SyncStatisticsPanel({ integrations }: SyncStatisticsProps) {
    ...
    try {
      if (selectedIntegration === 'all') {
        ...
        for (const integration of integrations) {
          ...
          for (const [entityType, entityStats] of Object.entries(
            stats.by_entity_type
          )) {
            // Runtime check for valid entity type keys
            if (entityType in allStats.by_entity_type) {
              const typedEntityType =
                entityType as keyof typeof allStats.by_entity_type
              allStats.by_entity_type[typedEntityType].count +=
                entityStats.count
              allStats.by_entity_type[typedEntityType].records +=
                entityStats.records
              allStats.by_entity_type[typedEntityType].errors +=
                entityStats.errors
            } else {
        ...
        if (countWithDuration > 0) {
          allStats.average_duration_ms = Math.round(
  ...
  const successRate =
    statistics && statistics.total_syncs > 0
      ? (statistics.successful_syncs / statistics.total_syncs) * 100
      : 0

  const getTrend = (current: number, previous: number) => {
    if (previous === 0)
      return { icon: Minus, color: 'text-muted-foreground', text: 'N/A' }
  ...
  const entityChartData = statistics
    ? Object.entries(statistics.by_entity_type)
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {Object.entries(statistics?.by_entity_type || {}).map(
              ([entity, stats]) => (
                <div key={entity} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      ...
                      style={{
                        backgroundColor: COLORS[entity as keyof typeof COLORS],
                      }}
                    />
