...
export function RevenueImpactCard({ revenueImpact }: RevenueImpactCardProps) {
  ...
  const impactLevel =
    revenueImpact.accuracyImprovement >= 5
      ? 'High Impact'
    ...
    <Card className="bg-gradient-to-br from-green-50 to-emerald-50 border-green-200">
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Badge className={impactColor}>{impactLevel}</Badge>
        </div>
      ...
      <CardContent>
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-green-700">Improvement over period</p>
          </div>
        ...
        <div className="mt-6 p-4 bg-white rounded-lg border border-green-200">
          <p className="text-sm text-green-800">
            <span className="font-semibold">ROI Highlight:</span> Your improved
            data accuracy is saving an average of{' '}
            <span className="font-bold">
              $
              {(
                revenueImpact.totalSaved /
                Math.max(revenueImpact.errorsPrevented, 1)
              ).toFixed(0)}
            </span>{' '}
