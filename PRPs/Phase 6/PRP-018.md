...
export class AnalyticsCalculator {
  ...
  async calculateOrderAccuracy(
    ...
    metricsByDate.forEach(metrics => {
      metrics.accuracyRate = metrics.totalOrders > 0
  ...
  async calculateSyncPerformance(
    ...
    syncLogs.forEach(log => {
      ...
      // Update success rate
      const successCount = syncLogs.filter(l => 
        format(new Date(l.started_at), 'yyyy-MM-dd') === date && 
        l.status === 'completed'
      ).length
      
      metrics.successRate = (successCount / metrics.syncCount) * 100
  ...
  async calculateInventoryTrends(
    ...
    snapshots.forEach(snapshot => {
      ...
      } else if (snapshot.quantity < 10) { // Configurable threshold
  ...
  async calculateRevenueImpact(
    ...
    const avgAccuracyBefore = beforeMetrics.reduce((sum, m) => sum + m.accuracyRate, 0) / beforeMetrics.length
    const avgAccuracyAfter = afterMetrics.reduce((sum, m) => sum + m.accuracyRate, 0) / afterMetrics.length
    const accuracyImprovement = avgAccuracyAfter - avgAccuracyBefore

    // Calculate errors prevented
    const totalOrdersAfter = afterMetrics.reduce((sum, m) => sum + m.totalOrders, 0)
    const errorsPrevented = Math.floor(totalOrdersAfter * (accuracyImprovement / 100))
    ...
    const projectedAnnualSavings = (totalSaved / daysInRange) * 365
  ...
  async cacheMetrics(
    ...
    if (metrics.syncPerformance) {
      upserts.push({
        ...
        avg_latency_ms: metrics.syncPerformance.avgLatency,
    ...
    if (upserts.length > 0) {
      await this.supabase
        ...
        .upsert(upserts, { onConflict: 'organization_id,metric_type,metric_date' })
    }
  }
}
```
...
export function MetricsCards({
  ...
  const currentAccuracy = orderAccuracy[orderAccuracy.length - 1]?.accuracyRate || 0
  const previousAccuracy = orderAccuracy[orderAccuracy.length - 8]?.accuracyRate || 0
  const accuracyChange = currentAccuracy - previousAccuracy

  const avgSyncTime = syncPerformance.reduce((sum, m) => sum + m.avgDuration, 0) / syncPerformance.length
  const totalInventoryValue = inventoryTrends[inventoryTrends.length - 1]?.totalValue || 0
...
export async function exportAnalytics({
  ...
  try {
    ...
    if (exportFormat === 'csv') {
      ...
      const csvData = orderAccuracy.map((accuracy, index) => ({
        date: accuracy.date,
        order_accuracy: accuracy.accuracyRate,
        total_orders: accuracy.totalOrders,
        error_count: accuracy.errorCount,
        sync_count: syncPerformance[index]?.syncCount || 0,
        avg_sync_duration: syncPerformance[index]?.avgDuration || 0,
        inventory_value: inventoryTrends[index]?.totalValue || 0,
        low_stock_count: inventoryTrends[index]?.lowStockCount || 0
      }))
