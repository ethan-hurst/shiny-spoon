...
export class AnalyticsCalculator {
  ...
  async calculateSyncPerformance(
    ...
    metricsByDate.forEach((metrics, date) => {
      const logsForDate = syncLogs.filter(log => 
        format(new Date(log.started_at), 'yyyy-MM-dd') === date
      )
  ...
  async calculateRevenueImpact(
    ...
    const avgAccuracyBefore = beforeMetrics.length > 0 
      ? beforeMetrics.reduce((sum, m) => sum + m.accuracyRate, 0) / beforeMetrics.length 
      : 0
    const avgAccuracyAfter = afterMetrics.length > 0 
      ? afterMetrics.reduce((sum, m) => sum + m.accuracyRate, 0) / afterMetrics.length 
      : 0
    ...
    const projectedAnnualSavings = daysInRange > 0 ? (totalSaved / daysInRange) * 365 : 0
...
export async function exportAnalytics({
  ...
  try {
    ...
    if (exportFormat === 'csv') {
      ...
      const csvData = Array.from(dateMap.values()).sort((a, b) => 
