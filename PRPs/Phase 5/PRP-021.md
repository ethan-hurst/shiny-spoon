  ...
  async calculateReorderPoints(
    ...
    for (const item of inventory) {
      ...
      const avgDailyDemand =
        forecast.predictions.reduce((a, b) => a + b) /
...
export class PriceOptimizer {
  ...
  private async calculateDemandElasticity(product: any): Promise<number> {
    ...
    for (let i = 1; i < priceHistory.length; i++) {
      ...
      const quantityDiff =
        (priceHistory[i].quantity_sold - priceHistory[i - 1].quantity_sold) /
        priceHistory[i - 1].quantity_sold
  ...
  private calculateOptimalPrice(params: {
    ...
    const { currentPrice, cost, elasticity, competitorPrices, inventoryLevel } =
      params
    ...
    // Combine factors
    let optimalPrice =
      markupPrice * 0.4 + avgCompetitorPrice * 0.4 + currentPrice * 0.2
...
export class AnomalyDetector {
  ...
  private async detectInventoryAnomalies(
    ...
    for (const change of changes) {
      ...
      const recentAdjustments = changes.filter(
        (c) =>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Intelligent predictions and recommendations powered by machine
            learning
          </p>
        </div>
        ...
        <Tabs defaultValue="forecast" className="space-y-4">
          ...
          <TabsContent value="trends" className="space-y-4">
            <TrendAnalysis
              organizationId={profile.organization_id}
              insights={
                insights.data?.filter((i) => i.insight_type === 'trend') || []
              }
            />
          </TabsContent>

          <TabsContent value="chat" className="space-y-4">
            <NaturalLanguageChat organizationId={profile.organization_id} />
          </TabsContent>
        </Tabs>
      </Suspense>
    </div>
  ...
  try {
    ...
    switch (type) {
      ...
      case 'demand-forecast':
        result = await generateDemandForecast(
          supabaseClient,
          organizationId,
          params
        )
...
function summarizeInventory(inventory: any[]) {
  ...
  return {
    ...
    totalValue: inventory.reduce(
      (sum, i) => sum + (i.quantity * i.products?.unit_price || 0),
      0
    ),
    lowStockItems: inventory.filter((i) => i.quantity < (i.reorder_point || 10))
      .length,
...
async function getBusinessContext(supabase: any, organizationId: string) {
  ...
  return {
    ...
    recentOrdersSummary: {
      ...
      totalValue:
        recentOrders.data?.reduce((sum: number, o: any) => sum + o.total, 0) ||
        0,
    },
