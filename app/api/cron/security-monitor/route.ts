  ...
  try {
    ...
    for (const org of organizations) {
      try {
        ...
        const suspiciousActivity = await accessControl.detectSuspiciousActivity(
          org.id
        )

        if (suspiciousActivity.suspiciousIPs.length > 0) {
          console.log(
            `Suspicious activity detected for org ${org.id}:`,
            suspiciousActivity
          )
        }
    ...
    // Log summary
    console.log(
      `Security monitoring completed: ${processedOrganizations} orgs, ${totalAlerts} alerts, ${criticalAlerts} critical`
    )
