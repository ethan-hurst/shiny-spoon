...
import type { AnomalyResult, DiscrepancyResult } from '@/lib/monitoring/types'
...
describe('AnomalyDetector', () => {
  ...
  describe('detectAnomalies', () => {
    ...
    it('should handle insufficient historical data', async () => {
      ...
      const statisticalAnomalies = anomalies.filter(
        (a) => a.anomalyType === 'statistical'
      )
  ...
  describe('detectStatisticalAnomalies', () => {
    it('should detect high z-score anomalies', () => {
      ...
      const statisticalAnomaly = anomalies.find(
        (a) => a.anomalyType === 'statistical'
      )
    ...
    it('should calculate z-score correctly', () => {
      ...
      const stdDev = (anomalyDetector as any).calculateStandardDeviation(
        values,
        mean
      )
      ...
      for (let week = 0; week < 4; week++) {
        for (let day = 0; day < 7; day++) {
          ...
          weeklyHistoricalData.push({
            ...
            discrepancyType: 'mismatch',
            count:
              day >= 1 && day <= 5 ? 10 + Math.random() * 2 : 2 + Math.random(),
            avgSeverity: 0.5,
            timestamp: date,
          })
        }
  ...
  describe('detectThresholdAnomalies', () => {
    ...
    it('should detect high confidence mismatches', () => {
      const highConfidenceDiscrepancies = Array(10)
        .fill(null)
        .map(() => ({
          ...mockDiscrepancies[0],
    ...
    it('should detect high stale data percentage', () => {
      const staleDiscrepancies = Array(10)
        .fill(null)
        .map((_, i) => ({
          ...mockDiscrepancies[0],
  ...
  describe('Time pattern detection', () => {
    ...
    it('should return null for insufficient data', () => {
      ...
      const pattern = (anomalyDetector as any).detectTimePattern(
        insufficientData
      )
  ...
  describe('zScoreToConfidence', () => {
    it('should map z-scores to confidence levels correctly', () => {
      const testCases = [
        ...
        { zScore: 2.2, expectedConfidence: 0.9 },
        { zScore: 1.7, expectedConfidence: 0.8 },
        { zScore: 1.0, expectedConfidence: 0.7 },
  ...
  describe('deduplicateAndRankAnomalies', () => {
    it('should remove duplicates keeping highest confidence', () => {
      ...
      const deduplicated = (anomalyDetector as any).deduplicateAndRankAnomalies(
        anomalies
      )
    ...
    it('should sort by confidence descending', () => {
      ...
      const sorted = (anomalyDetector as any).deduplicateAndRankAnomalies(
        anomalies
      )
