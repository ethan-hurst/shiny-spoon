...
describe('ConflictResolutionService', () => {
  ...
  describe('detectConflicts', () => {
    ...
    it('should not detect timestamp conflicts for minor differences', async () => {
      const localData = {
        ...mockLocalData,
        updated_at: '2024-01-15T10:00:00.500Z',
      }
      const externalData = {
        ...mockExternalData,
        updated_at: '2024-01-15T10:00:00.800Z',
      }
    ...
    it('should detect data field conflicts', async () => {
      ...
      expect(quantityConflict).toEqual({
        ...
        external_value: 95,
        description:
          'quantity values differ between local and external systems',
      })
    ...
    it('should ignore fields not in comparison list', async () => {
      ...
      const descriptionConflict = conflicts.find(
  ...
  describe('resolveConflicts', () => {
    ...
    it('should handle multiple conflicts with mixed results', async () => {
      ...
      jest
        .spyOn(conflictService as any, 'applyResolution')
        .mockImplementationOnce(() => Promise.resolve({ quantity: 100 }))
        .mockImplementationOnce(() =>
          Promise.reject(new Error('Invalid field'))
        )
  ...
  describe('applyResolution', () => {
    it('should apply LOCAL_WINS resolution', async () => {
      const result = await (conflictService as any).applyResolution(
        mockConflict,
        ConflictResolution.LOCAL_WINS
      )
    ...
    it('should throw error for MANUAL resolution', async () => {
      await expect(
        (conflictService as any).applyResolution(
          mockConflict,
          ConflictResolution.MANUAL
        )
  ...
  describe('getConflictHistory', () => {
    ...
    it('should handle database errors', async () => {
      ...
      await expect(
        conflictService.getConflictHistory('sync-123')
      ...
      mockSupabase.from.mockReturnValue({
        select: jest.fn().mockReturnValue({
          eq: jest.fn().mockReturnValue({
            order: jest
              .fn()
              .mockImplementation((field: string, options: any) => {
