...
describe('NetSuiteTransformers', () => {
  ...
  describe('transformProduct', () => {
    ...
    it('should throw error on transformation failure', async () => {
      ...
      await expect(transformers.transformProduct(invalidItem)).rejects.toThrow(
        'Failed to transform product PROD-001:'
      )
    })
    ...
    it('should apply field mappings', async () => {
      ...
      const result =
        await mappedTransformers.transformProduct(itemWithMappedFields)
  ...
  describe('transformInventory', () => {
    ...
    it('should transform inventory balance successfully', async () => {
      const result = await transformers.transformInventory(
        mockBalance,
        mockLocation
      )
    ...
    it('should truncate long warehouse codes', async () => {
      ...
      expect(result.warehouse_code).toMatch(
        /^THIS_IS_A_VERY_LONG_WAREHOUSE_NAME_THAT_EXCEEDS_T/
      )
    })
    ...
    it('should throw error on transformation failure', async () => {
      ...
      await expect(
        transformers.transformInventory(invalidBalance, mockLocation)
  ...
  describe('transformSalesOrder', () => {
    ...
    it('should transform sales order with lines successfully', async () => {
      const result = await transformers.transformSalesOrder(
        mockOrder,
        mockLines
      )
    ...
    it('should map order statuses correctly', async () => {
      const statusMappings = [
        ...
        {
          netsuiteStatus: 'Partially Fulfilled',
          expected: 'partially_shipped',
        },
        {
          netsuiteStatus: 'Pending Billing/Partially Fulfilled',
          expected: 'partially_shipped',
        },
  ...
  describe('date parsing', () => {
    ...
    it('should throw error for empty date', async () => {
      ...
      await expect(transformers.transformProduct(item)).rejects.toThrow(
        'NetSuite date is empty or undefined'
      )
    })
    ...
    it('should throw error for invalid date format', async () => {
      ...
      await expect(transformers.transformProduct(item)).rejects.toThrow(
        'Invalid NetSuite date format: "invalid-date"'
      )
    })
  })
  ...
  describe('utility methods', () => {
    ...
    it('should handle empty parts in composite key', () => {
      const key = transformers.buildCompositeKey(
        '',
        0,
        null as any,
        undefined as any
    ...
    it('should throw validation error for invalid data', () => {
      ...
      expect(() =>
        transformers.validateTransformedData(invalidData, schema)
