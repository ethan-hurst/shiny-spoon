...
describe('Pricing Server Actions', () => {
  ...
  describe('createProductPricing', () => {
    ...
    it('should throw error when unauthorized', async () => {
      ...
      await expect(createProductPricing(formData as any)).rejects.toThrow('Unauthorized')
    })
  })
  ...
  describe('createPricingRule', () => {
    it('should create pricing rule with quantity breaks', async () => {
      ...
      formData.append('quantity_breaks', JSON.stringify([
        { min_quantity: 10, max_quantity: 50, discount_type: 'percentage', discount_value: 5 },
        { min_quantity: 51, max_quantity: null, discount_type: 'percentage', discount_value: 10 }
      ]))
    ...
    it('should update multiple customer prices', async () => {
      ...
      mockSupabase.from
        .mockReturnValueOnce({
          ...
          eq: jest.fn().mockReturnThis(),
          single: jest.fn().mockResolvedValue({
            data: { id: 'product-1-id', product_pricing: { base_price: 100, cost: 50 } },
            error: null
          })
        })
        .mockReturnValueOnce({
        ...
        .mockReturnValueOnce({
          ...
          eq: jest.fn().mockReturnThis(),
          single: jest.fn().mockResolvedValue({
            data: { id: 'product-2-id', product_pricing: { base_price: 200, cost: 100 } },
            error: null
          })
        })
        .mockReturnValueOnce({
