...
describeWithSupabase('RLS Policies', () => {
  ...
  beforeAll(async () => {
    ...
    await adminSupabase.auth.admin
      ...
      .catch((error: any) => {
        console.warn(
          'Failed to delete test user 1 during cleanup:',
          error.message
        )
      })
    await adminSupabase.auth.admin
      ...
      .catch((error: any) => {
        console.warn(
          'Failed to delete test user 2 during cleanup:',
          error.message
        )
      })
  ...
  afterAll(async () => {
    ...
    await adminSupabase
      .from('products')
      .delete()
      .in('id', ['product-1-id', 'product-2-id'])
    await adminSupabase
      .from('user_profiles')
      .delete()
      .in('organization_id', [user1.organizationId, user2.organizationId])
    await adminSupabase
      .from('organizations')
      .delete()
      .in('id', [user1.organizationId, user2.organizationId])
