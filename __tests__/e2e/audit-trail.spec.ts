...
test.describe('Audit Trail', () => {
    ...
    await page.route('**/rest/v1/audit_logs_with_details**', async (route) => {
      ...
      if (searchParams.get('entity_type')) {
        ...
        filteredLogs = filteredLogs.filter(
          (log) => log.entity_type === entityType
        )
      }
      ...
      await route.fulfill({
        json: filteredLogs,
        headers: {
          'Content-Range': `0-${filteredLogs.length - 1}/${filteredLogs.length}`,
        },
      })
    })

    await page.route(
      '**/rest/v1/user_profiles?select=user_id,full_name,email**',
      async (route) => {
        await route.fulfill({
          json: mockUsers,
        })
  ...
  test('should display audit trail page with correct title and description', async ({
    page,
  }) => {
  ...
  test('should display audit logs in table format', async ({ page }) => {
    ...
    await expect(
      firstRow.locator('[data-testid="action-badge"]')
    ).toContainText('create')
    await expect(firstRow.locator('td').nth(3)).toContainText('product')
  })

  test('should show user information with avatars and roles', async ({
    page,
  }) => {
    // Check that user avatars are displayed
    await expect(
      page.locator('tbody tr [data-testid="user-avatar"]')
    ).toHaveCount(3)

    // Check user names and roles are displayed
    await expect(
      page.locator('tbody tr').first().locator('[data-testid="user-info"]')
    ).toContainText('Admin User')
    await expect(
      page.locator('tbody tr').first().locator('[data-testid="user-role"]')
    ).toContainText('admin')

    await expect(
      page.locator('tbody tr').nth(1).locator('[data-testid="user-info"]')
    ).toContainText('Regular User')
    await expect(
      page.locator('tbody tr').nth(1).locator('[data-testid="user-role"]')
    ).toContainText('member')
  })

  test('should display action badges with correct colors and icons', async ({
    page,
  }) => {
    // Check action badges
    const createBadge = page
      .locator('tbody tr')
      .first()
  ...
  test('should filter audit logs by user', async ({ page }) => {
    ...
    // Select a user
    await page
      .locator('[data-testid="user-option"]:has-text("Admin User")')
      .click()
  ...
  test('should filter audit logs by action type', async ({ page }) => {
    ...
    await page
      .locator('[data-testid="action-option"]:has-text("Create")')
      .click()
  ...
  test('should filter audit logs by entity type', async ({ page }) => {
    ...
    await page
      .locator('[data-testid="entity-option"]:has-text("Product")')
      .click()
  ...
  test('should show retention policy button for admin users', async ({
    page,
  }) => {
    // Check that retention policy button is visible for admin
    await expect(
      page.locator('[data-testid="retention-policy-button"]')
    ).toBeVisible()
  })
  ...
  test('should open retention policy dialog', async ({ page }) => {
    ...
    await expect(
      page.locator('h2:has-text("Audit Log Retention Policies")')
    ).toBeVisible()
  ...
  test('should add new retention policy', async ({ page }) => {
    ...
    await page
      .locator('[data-testid="entity-option"]:has-text("Products")')
      .click()
    ...
    await expect(
      page.locator('[data-testid="retention-table"] tbody tr')
    ).toHaveCount(2)
  })
  ...
  test('should navigate between pages', async ({ page }) => {
    ...
    await page.route('**/rest/v1/audit_logs_with_details**', async (route) => {
      ...
      const logs = Array.from(
        { length: Math.min(pageSize, totalLogs - start) },
        (_, i) => ({
          id: `log-${start + i + 1}`,
      ...
      await route.fulfill({
        json: logs,
        headers: {
          'Content-Range': `${start}-${start + logs.length - 1}/${totalLogs}`,
        },
      })
    })
  ...
  test('should handle loading state', async ({ page }) => {
    ...
    await expect(page.locator('table')).toBeVisible()
    await expect(
      page.locator('[data-testid="audit-skeleton"]')
    ).not.toBeVisible()
  })
