...
const describePerformance = process.env.RUN_PERF_TESTS ? describe : describe.skip
...
describePerformance('Pricing Engine Performance', () => {
  ...
  describe('Single Price Calculations', () => {
    ...
    it('should handle 100 concurrent price calculations', async () => {
      ...
      console.log(`Total time for 100 concurrent calculations: ${duration.toFixed(2)}ms`)
  ...
  describe('Batch Price Calculations', () => {
    ...
    it('should outperform sequential calculations for multiple products', async () => {
      ...
      console.log(`Improvement: ${((sequentialDuration - batchDuration) / sequentialDuration * 100).toFixed(1)}%`)
  ...
  describe('Cache Performance', () => {
    it('should significantly improve repeated calculations', async () => {
      ...
      console.log(`Second call (potential cache hit): ${secondDuration.toFixed(2)}ms`)
  ...
  describe('Memory Usage', () => {
    it('should handle large batch calculations without excessive memory', async () => {
      ...
      console.log(`Memory increase for 1000 products: ${memoryIncrease.toFixed(2)}MB`)
  ...
  describe('Stress Testing', () => {
    it('should handle high-frequency requests', async () => {
      ...
      while (performance.now() - start < duration) {
        await getQuickPrice(`product-${requestCount % 100}`, 'test-customer', 10)
        requestCount++
      }
      ...
      console.log(`Completed ${requestCount} requests in ${actualDuration.toFixed(0)}ms`)
    ...
    it('should maintain performance under concurrent load', async () => {
      ...
      const userSimulations = Array.from({ length: concurrentUsers }, async (_, userId) => {
        ...
        for (let i = 0; i < requestsPerUser; i++) {
          await getQuickPrice(`product-${i}`, `user-${userId}`, Math.random() * 100)
      ...
      const avgUserDuration = userDurations.reduce((a, b) => a + b, 0) / concurrentUsers
      
      console.log(`Total duration: ${totalDuration.toFixed(0)}ms`)
      console.log(`Average user completion time: ${avgUserDuration.toFixed(0)}ms`)
...
describePerformance('Performance Report', () => {
  it('should generate performance summary', async () => {
    ...
    if (metrics.singleCalculation > 20) {
      console.log('- Consider optimizing database queries for single price lookups')
    }
    ...
    console.log('- Consider implementing a price calculation queue for bulk operations')
  })
})