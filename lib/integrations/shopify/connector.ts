...
export class ShopifyConnector extends BaseConnector {
  ...
  async syncInventory(): Promise<SyncResult> {
    try {
      ...
      const inventoryUpdates = inventoryLevels
        .map((level) => {
          const mapping = mappings?.find(
            (m) => m.shopify_variant_id === level.inventory_item_id.toString()
          )
          return {
      ...
      console.log(
        `Synced ${inventoryUpdates.length} inventory levels from Shopify`
      )
  ...
  async syncCustomers(): Promise<SyncResult> {
    try {
      ...
      console.log(
        `Synced ${transformedCustomers.length} customers from Shopify`
      )
  ...
  async syncB2BCatalogs(): Promise<SyncResult> {
    try {
      ...
      console.log(
        `Synced ${transformedCatalogs.length} B2B catalogs from Shopify`
      )
  ...
  private async handleInventoryWebhook(
    ...
    console.log(
      'Processing inventory webhook for variant:',
  ...
  async updateSyncState(
    ...
    try {
      await this.supabase.from('shopify_sync_state').upsert(
        {
          integration_id: this.config.integrationId,
