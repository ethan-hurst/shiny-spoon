...
export class RealtimeConnectionManager {
  ...
  private calculateStability(): number {
    ...
    const variance =
      this.metrics.latency.reduce((sum, latency) => {
  ...
  public getHealthScore(): number {
    ...
    return Math.round(
      (latencyScore + stabilityScore + uptimeScore + reconnectScore) / 4
    )
  }
  ...
  public getRecommendations(): string[] {
    ...
    if (this.status.reconnectAttempts > 2) {
      recommendations.push(
        'Multiple reconnection attempts. Check if the service is available.'
      )
    }
