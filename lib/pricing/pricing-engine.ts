...
export class PricingEngine {
  ...
  async getApplicableRules(
    ...
    const { data: rules, error } = await supabase
      ...
      .eq('is_active', true)
      .or(
        `start_date.is.null,start_date.lte.${context.date.toISOString().split('T')[0]}`
      )
      .or(
        `end_date.is.null,end_date.gte.${context.date.toISOString().split('T')[0]}`
      )
  ...
  private isRuleApplicable(
    ...
    // Tier-specific rule
    if (
      rule.customer_tier_id &&
      rule.customer_tier_id !== context.customerTier
    ) {
      return false
    }
  ...
  private evaluateConditions(conditions: any, context: PriceContext): boolean {
    ...
    if (conditions.customer_tiers && Array.isArray(conditions.customer_tiers)) {
      if (
        ...
        !conditions.customer_tiers.includes(context.customerTier)
      ) {
        return false
      }
    }
    ...
    if (
      ...
      Array.isArray(conditions.product_categories)
    ) {
      if (
        !context.productCategory ||
        !conditions.product_categories.includes(context.productCategory)
      ) {
        return false
      }
    }
    ...
    for (const [key, value] of Object.entries(custom)) {
      switch (key) {
        ...
        case 'min_available_inventory':
          if (
            !context.inventory ||
            context.inventory.availableQuantity < value
          ) {
            return false
          }
          break
        ...
        case 'inventory_level':
          ...
          const inventoryPercent =
            (context.inventory.availableQuantity /
              context.inventory.totalQuantity) *
            100
  ...
  applyRule(
    ...
    if (rule.rule_type === 'quantity' && rule.quantity_breaks) {
      const applicableBreak = rule.quantity_breaks.find(
        (qb) =>
          context.quantity >= qb.min_quantity &&
      ...
      const batchResults = await Promise.all(
        batch.map(async (request) => {
          try {
            ...
            console.error(
              `Failed to calculate price for product ${request.product_id}:`,
              error
            )
            return null
          }
