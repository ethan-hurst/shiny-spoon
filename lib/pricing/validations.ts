...
export const createProductPricingSchema = productPricingSchema.extend({
...
export const updateProductPricingSchema = productPricingSchema.partial().extend({
  id: z.string().uuid(),
})
...
export const updatePricingRuleSchema = pricingRuleSchema.partial().extend({
  id: z.string().uuid(),
  quantity_breaks: z.array(quantityBreakSchema.extend({
...
export const updateCustomerPricingSchema = customerPricingSchema.partial().extend({
  id: z.string().uuid(),
})
...
export function validatePriceGreaterThanCost(price: number, cost: number): boolean {
  return price > cost
}

export function validateMargin(price: number, cost: number, minMargin: number): boolean {
...
export function validateQuantityBreaks(breaks: QuantityBreak[]): string[] {
  ...
  for (let i = 0; i < sortedBreaks.length; i++) {
    ...
    if (current.max_quantity && current.max_quantity <= current.min_quantity) {
      errors.push(`Break ${i + 1}: Max quantity must be greater than min quantity`)
    }
    ...
    if (next) {
      if (current.max_quantity) {
        if (next.min_quantity > current.max_quantity) {
          errors.push(`Gap between quantities ${current.max_quantity} and ${next.min_quantity}`)
        }
        if (next.min_quantity < current.max_quantity) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (discount.endsWith('%')) {
      ...
      if (discount.includes('off')) {
        discountType = 'fixed'
        discountValue = parseFloat(discount.replace('$', '').replace('off', '').trim())
      } else {
        discountType = 'price'
...
export function transformPricingRuleImport(data: z.infer<typeof pricingRuleImportSchema>): {
...
export function transformProductPricingImport(data: z.infer<typeof productPricingImportSchema>): Partial<ProductPricing> {
  return {
    cost: data.cost,
...
export function checkRuleConflicts(
  ...
  for (const existing of existingRules) {
    ...
    if (newRule.product_id && existing.product_id && newRule.product_id !== existing.product_id) {
      continue
    }
    
    // Skip if different categories
    if (newRule.category_id && existing.category_id && newRule.category_id !== existing.category_id) {
      continue
    }
    ...
    const existingStart = existing.start_date ? new Date(existing.start_date) : null
    ...
    if (hasDateOverlap && existing.rule_type === newRule.rule_type) {
      ...
      if (existing.priority === newRule.priority) {
        conflicts.push(`Conflicts with rule "${existing.name}" - same priority (${existing.priority})`)
      }
