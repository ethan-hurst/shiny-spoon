...
async function checkRLSEnabled(supabase, tableName) {
  console.log(`${colors.blue}Checking RLS for table: ${tableName}${colors.reset}`)
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42883' || error.details?.includes('function') || error.message.includes('does not exist')) {
        ...
        if (retryError) {
          return { success: false, error: `RLS check failed: ${retryError.message}` }
        }
...
async function validateMigrations() {
  console.log(`${colors.yellow}🔍 Running PRP-002 Migration Validation...${colors.reset}\n`)
  ...
  if (dirCheck.success) {
    console.log(`${colors.green}✅ Found ${dirCheck.migrations.length} migration files${colors.reset}`)
    results.push({ test: 'Migrations Directory', passed: true })
  } else {
    console.log(`${colors.red}❌ ${dirCheck.error}${colors.reset}`)
    results.push({ test: 'Migrations Directory', passed: false, error: dirCheck.error })
    allPassed = false
  }
  ...
  try {
    ...
    console.log(`${colors.yellow}Make sure NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are set${colors.reset}`)
    process.exit(1)
  }
  ...
  for (const table of coreTables) {
    ...
    if (result.success) {
      ...
      results.push({ test: `Table: ${table}`, passed: false, error: result.error })
      allPassed = false
    }
  }
  ...
  for (const table of rlsTables) {
    ...
    if (result.success) {
      ...
      results.push({ test: `RLS: ${table}`, passed: false, error: 'RLS not enabled' })
      allPassed = false
    }
  }
  ...
  try {
    ...
    console.log(`${colors.yellow}⚠️  No seed.sql file found (optional)${colors.reset}`)
  }
