-- Fix schema search path issues in the trigger function
-- This ensures all table references use explicit schema names
...
BEGIN
  ...
  IF NEW.raw_user_meta_data->>'invited_organization_id' IS NULL THEN
    BEGIN
      INSERT INTO public.audit_retention_policies (organization_id, entity_type, retention_days)
      VALUES (org_id, NULL, 365)
      ON CONFLICT (organization_id, entity_type) DO NOTHING;
    EXCEPTION WHEN OTHERS THEN
      -- If any error occurs, log it but don't fail the entire registration
