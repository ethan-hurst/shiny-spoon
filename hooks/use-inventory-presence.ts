...
export function useInventoryPresence({ inventoryId, currentView }: UseInventoryPresenceOptions) {
  ...
  const getActiveUsers = useCallback((): PresenceData[] => {
    ...
    return presenceData.filter(presence => 
      new Date(presence.lastActivity) > twoMinutesAgo
    )
  }, [presenceData])

  const getUsersOnItem = useCallback((itemId: string): PresenceData[] => {
    return getActiveUsers().filter(presence => 
