...
export function useWarehouses(initialData?: Warehouse[]) {
  ...
  const fetchWarehouses = useCallback(
    async (currentFilters?: WarehouseFilters) => {
      setIsLoading(true)
      try {
      ...
      `)
        ...
        if (filtersToUse.search) {
          query = query.or(
            `code.ilike.%${filtersToUse.search}%,name.ilike.%${filtersToUse.search}%`
          )
        }
        ...
        if (rpcError) {
          ...
          try {
            ...
            if (setError) {
              ...
              console.error(
                'Failed to set new default, attempting to restore previous default:',
                setError
              )
              ...
              if (previousDefaultId) {
                ...
                if (restoreError) {
                  console.error(
                    'Critical: Failed to restore previous default warehouse:',
                    restoreError
                  )
                  toast.error(
                    'Failed to update default warehouse and could not restore previous state. Please refresh and try again.'
                  )
                } else {
                  toast.error(
                    'Failed to set new default warehouse. Previous default has been restored.'
                  )
                }
              } else {
                toast.error(
                  'Failed to set new default warehouse. No previous default to restore.'
                )
              }
            ...
            if (previousDefaultId) {
              ...
              if (restoreError) {
                ...
                toast.error(
                  'Operation failed and could not restore previous state. Please refresh and try again.'
                )
              } else {
                toast.error(
                  'Operation failed. Previous default has been restored.'
                )
              }
            }
            throw error
